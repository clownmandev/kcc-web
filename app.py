...
def worker_process(job_id, params):
    try:
        ...
        # --- OPTION A: ONE HUGE FILE ---
        if combine and mode in ['mangadex', 'scraper']:
            master_path = os.path.join(DOWNLOAD_FOLDER, f"full_{job_id}")
            os.makedirs(master_path, exist_ok=True)
            url = params.get('url') if mode == 'scraper' else f"https://mangadex.org/title/{params.get('manga_id')}"
            
            log_to_job(job_id, f"--- Combining into ONE file: {m_title} ---")
            dl_cmd = ['mangadex-downloader', url, '--language', 'en', '--folder', master_path, '--no-group-name', 
                      '--start-volume', params.get('vol_start'), '--end-volume', params.get('vol_end')]
            ...
            run_command(['kcc-c2e', '-p', profile, '-f', format_type, '--output', job_work_dir, master_path], job_id)
            shutil.rmtree(master_path)

        # --- OPTION B: SEPARATE FILES ---
        elif mode in ['mangadex', 'scraper']:
            url = params.get('url') if mode == 'scraper' else f"https://mangadex.org/title/{params.get('manga_id')}"
            v_start, v_end = int(params.get('vol_start', 1)), int(params.get('vol_end', 1))
            
            for v in range(v_start, v_end + 1):
                log_to_job(job_id, f"--- Volume {v} (Separate Mode) ---")
                ...
                if inputs: run_command(['kcc-c2e', '-p', profile, '-f', format_type, '--output', job_work_dir] + inputs, job_id)
        ...
        for root, dirs, files in os.walk(job_work_dir):
            for f in files:
                if f.lower().endswith(('.epub', '.mobi', '.azw3')): found_books.append(os.path.join(root, f))

        if not found_books:
            log_to_job(job_id, "FAILED: No output generated."); JOBS[job_id]['status'] = 'failed'; return

        if len(found_books) == 1:
            final_name = f"{m_title}.{format_type.lower()}"
            shutil.move(found_books[0], os.path.join(OUTPUT_FOLDER, final_name))
            JOBS[job_id]['result'] = final_name
        else:
            zip_name = f"{m_title}_Collection.zip"
            zip_temp = os.path.join(DOWNLOAD_FOLDER, f"zip_{job_id}")
            os.makedirs(zip_temp, exist_ok=True)
            for b in found_books: shutil.copy(b, zip_temp)
            shutil.make_archive(os.path.join(OUTPUT_FOLDER, zip_name.replace('.zip','')), 'zip', zip_temp)
